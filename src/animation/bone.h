...
{
public:
	static void SetUseInterpolate(bool b_use_interpolate);
...
public:
	...
	float get_factor();
	void get_ai_node(aiNodeAnim* channel,
...
private:
	...
	template <class T>
	const std::pair<const T&, const T&> get_start_end(typename std::map<float, T>& mp,
	...
	float factor_{1.0f};
	static bool b_use_interpolate_;
};
...
template <class T>
const std::pair<const T&, const T&> Bone::get_start_end(typename std::map<float, T>& mp,
														float animation_time,
														const T& default_value)
{
	if (mp.size() == 0)
